...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def chunk_text(self, text: str, metadata: Dict[str, Any]) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        for sentence in sentences:
            ...
            if len(test_chunk) <= self.chunk_size:
                ...
                # Include previous sentences for context (up to overlap size)
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]], 
        ...
        # Handle replace mode: delete all existing documents from this source
        if update_mode == 'replace':
            deleted = self.delete_documents_by_source(file_path.stem)
            logger.info(f"Deleted {deleted} existing documents in replace mode")
        ...
        for i in range(0, len(documents), batch_size):
            ...
            for doc_text, metadata in batch:
                ...
                if exists:
                    if update_mode == 'skip':
                        ...
                        self.update_document(doc_id, doc_text, metadata)
                        stats['updated'] += 1
                        continue
                ...
                try:
                    ...
                    logger.warning(f"Failed to get embedding for {doc_id}, skipping")
                    continue
            ...
            if ids_batch:
                try:
                    ...
                    stats['added'] += len(ids_batch)
                    logger.debug(f"Added {len(ids_batch)} documents to collection")
                except Exception as e:
    ...
    def process_all_text_data(self, base_path: str, update_mode: str = 'skip') -> Dict[str, int]:
        ...
        for file_path in files:
            try:
                ...
                # Initialize mission stats if needed
                ...
                # Update statistics
                batch_stats = self.add_documents_to_collection(documents, file_path, update_mode=update_mode)
    ...
    def get_collection_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'document_count': len(all_docs['metadatas']) if all_docs['metadatas'] else 0
            }
        except Exception as e:
